...
fig_precmd() {
  ...
  fig_osc "FigAutosuggestionColor=%s" "${BAY_AUTOSUGGEST_HIGHLIGHT_STYLE}"
  fig_osc "User=%s" "${USER:-root}"

  if [ "$BAY_HAS_SET_PROMPT" -eq 1 ]; then
  ...
  NEW_CMD=$'\033]697;NewCmd='"${BAYTERM_SESSION_ID}"$'\007'

  # Save user defined prompts.
  BAY_USER_PS1="$PS1"
  BAY_USER_PROMPT="$PROMPT"
  BAY_USER_prompt="$prompt"

  BAY_USER_PS2="$PS2"
  BAY_USER_PROMPT2="$PROMPT2"

  BAY_USER_PS3="$PS3"
  BAY_USER_PROMPT3="$PROMPT3"

  BAY_USER_PS4="$PS4"
  BAY_USER_PROMPT4="$PROMPT4"

  BAY_USER_RPS1="$RPS1"
  BAY_USER_RPROMPT="$RPROMPT"

  BAY_USER_RPS2="$RPS2"
  BAY_USER_RPROMPT2="$RPROMPT2"

...
const ONBOARDING_MENU_ID: &str = "onboarding";
...
pub fn get_icon() -> Icon {
    #[cfg(target_os = "linux")]
    let bytes = include_bytes!("../icons/icon-monochrome-light.png").to_vec();
    #[cfg(not(target_os = "linux"))]
    let bytes = include_bytes!("../icons/icon-monochrome.png").to_vec();
...
fn menu() -> Vec<MenuElement> {
    ...
    let mut menu = if !onboarded_completed {
        ...
        vec![
            ...
            MenuElement::entry(None, None, "Get Started", ONBOARDING_MENU_ID),
        ]
    } else {
        vec![settings]
    };
